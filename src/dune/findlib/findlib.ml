...
let find_root_package t ~root_name =
  let rec loop dirs :
      (Root_package.t, Root_package.Unavailable_reason.t) Result.t =
    match dirs with
    | [] -> (
      ...
      | _ -> (
        ...
        | None -> Error Root_package.Unavailable_reason.Not_found
    ...
    | dir :: dirs -> (
      ...
      with
      | Error e ->
        Error (Root_package.Unavailable_reason.Invalid_dune_package e)
